...
namespace LitFramework
{
    ...
    public class Timer
    {
        ...
        public static TimerManager _manager;
        ...
        private Timer(float duration, Action onComplete, Action<float> onUpdate,
            bool isLooped, bool usesRealTime, MonoBehaviour autoDestroyOwner, bool canCancelByAll)
        {
            ...
            this._canCancelByAll = canCancelByAll;
        }
        ...
        public class TimerManager : MonoBehaviour
        {
            ...
            private List<Timer> _lateTimers = new List<Timer>();

            // buffer adding timers so we don't edit a collection during iteration
            private List<Timer> _lateTimersToAdd = new List<Timer>();
            ...
            public void RegisterLateTimer(Timer timer)
            {
                this._lateTimersToAdd.Add(timer);
            }

            public void CancelAllTimers()
            {
                List<Timer> lastTimer = new List<Timer>();
                
                foreach (Timer timer in this._timersToAdd)
                {
                    if (timer._canCancelByAll)
                    {
                        timer.Cancel();
                    }
                    else
                ...
                this._timers = lastTimer;
                this._timersToAdd = new List<Timer>();
                
                List<Timer> lastLateTimer = new List<Timer>();
                foreach (Timer timer in this._lateTimersToAdd)
                {
                    if (timer._canCancelByAll)
                    {
                        timer.Cancel();
                    ...
                    {
                        lastLateTimer.Add(timer);
                    }
                }
                
                foreach (Timer timer in this._lateTimers)
                {
                    if (timer._canCancelByAll)
                    {
                        timer.Cancel();
                ...
                this._lateTimers = lastLateTimer;
                this._lateTimersToAdd = new List<Timer>();
            }
            ...
            private void UpdateAllTimers()
            {
                try
                {
                    if (!this || this._timers == null || this._timersToAdd == null)
                    {
                        return;
                    }
            ...
            private void LateUpdateAllTimers()
            {
                try
                {
                    if (!this || this._lateTimers == null || this._lateTimersToAdd == null)
                    {
                        return;
                    }

                    if (this._lateTimersToAdd.Count > 0)
                    {
                        this._lateTimers.AddRange(this._lateTimersToAdd);
                        this._lateTimersToAdd.Clear();
                    }
                    ...
                    this._lateTimers.RemoveAll(t => t.isDone);
                }
                catch (Exception e)
                {
                    Debug.LogError(e);
            ...
            bool canCancelByAll = true)
        {
            ...
            if (Timer._manager == null)
            {
                ...
                else
                {
                    ...
                    managerObject.transform.localPosition = new Vector3(9999999, 9999999, 9999999);
        ...
        /// <param name="duration">The time to wait before the timer should fire, in seconds.</param>
        /// <param name="onComplete">An action to fire when the timer completes.</param>
        /// <param name="onLateUpdate">An action that should fire each time the timer is lateUpdated. Takes the amount
        /// of time passed in seconds since the start of the timer's current loop.</param>
        /// <param name="isLooped">Whether the timer should repeat after executing.</param>
        /// <param name="useRealTime">Whether the timer uses real-time(i.e. not affected by pauses,
        /// slow/fast motion) or game-time(will be affected by pauses and slow/fast-motion).</param>
        ...
        /// after the parent has been destroyed.</param>
        /// <returns>A timer object that allows you to examine stats and stop/resume progress.</returns>
        public static Timer RegisterLate(float duration, Action onComplete, Action<float> onLateUpdate = null,
            bool isLooped = false, bool useRealTime = false, MonoBehaviour autoDestroyOwner = null,
            bool canCancelByAll = true)
        {
            // create a manager object to update all the timers if one does not already exist.
            ...
            Timer timer = new Timer(duration, onComplete, onLateUpdate, isLooped, useRealTime, autoDestroyOwner,
                canCancelByAll);
            Timer._manager.RegisterLateTimer(timer);
            return timer;
        }


        public static Timer RegisterOnComplete(float duration, Action onComplete, MonoBehaviour autoDestroyOwner = null,
            bool canCancelByAll = true)
        {
            return Register(duration, onComplete, null, false, false, autoDestroyOwner, canCancelByAll);
        }

        public static Timer RegisterOnCompleteLoop(float duration, Action onComplete, MonoBehaviour autoDestroyOwner = null,bool canCancelByAll = true)
        {
            return Register(duration, onComplete, null, true, false, autoDestroyOwner,canCancelByAll);
        }
        
        public static Timer RegisterLateOnComplete(float duration, Action onComplete, MonoBehaviour autoDestroyOwner = null,
            bool canCancelByAll = true)
        {
            return RegisterLate(duration, onComplete, null, false, false, autoDestroyOwner, canCancelByAll);
        }

        public static Timer RegisterLateOnCompleteLoop(float duration, Action onComplete, MonoBehaviour autoDestroyOwner = null,bool canCancelByAll = true)
        {
            return RegisterLate(duration, onComplete, null, true, false, autoDestroyOwner,canCancelByAll);
        }
