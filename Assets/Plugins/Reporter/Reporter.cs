...
public class Reporter : MonoBehaviour {
    ...
    //jank
    private int currentJankSecond = 0;
    private int currentJankCount = 0;
    private float jankThreshold = 0.035f;
    private int[] jankHistory = new int[10];
    ...
    void addSample(){
        ...
        sample.loadedScene = (byte)Math.Max(SceneManager.GetActiveScene().buildIndex,0) ;
    ...
    void OnGUI ()
    {
        GUI.Label( new Rect(100, 0, 250, 50) , fpsText  );//lowerLeftFontStyle
    }
    ...
    void Update()
    {
        fpsText = fps.ToString("0.000") + " Jank: " + String.Join("-",jankHistory);
        gcTotalMemory = (((float)System.GC.GetTotalMemory(false))/1024/1024) ;
        //addSample();
        // if(SceneManager.GetActiveScene().buildIndex < scenes.Length && string.IsNullOrEmpty( scenes[ SceneManager.GetActiveScene().buildIndex ] ))
    ...
    /// <summary>
    /// 添加 jank 的统计. 超过阈值 35ms 的帧会记录一次 jank
    /// 如果一帧时间为阈值的多倍,则记录多次 jank ,如 73ms 视为 2 次 jank
    /// jank 会在 fps 后面显示,滚动显示 10 秒的 jank,最终汇总为总 jank 数
    /// </summary>
    void UpdateJank()
    {
        if (((int)Time.time) != currentJankSecond)
        {
            jankHistory[jankHistory.Length - 1] += jankHistory[jankHistory.Length - 2];
            //print  jankHistory
            for (int i = jankHistory.Length - 3; i >=0 ; i--)
            {
                jankHistory[i + 1] = jankHistory[i];
            }

            jankHistory[0] = currentJankCount;

            currentJankCount = 0;
            currentJankSecond = (int)Time.time;
        }

        if (Time.deltaTime > jankThreshold)
        {
            currentJankCount+= (int)(Time.deltaTime / jankThreshold) ;
        }
    }
